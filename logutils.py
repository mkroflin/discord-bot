...
def get_player_info(log_data, start_times, end_times, last_phase_id):
    player_names = []
    class_names = []
    start_dpses = []
    end_dpses = []
    phase_dpses = []
    phase_ids = []
    for phase_id, start, end in zip(range(last_phase_id, last_phase_id + len(start_times)), start_times, end_times):
        for player_id in range(len(log_data['players'])):
            player_name, class_name, start_dps, end_dps, phase_dps = get_player_dps_info(log_data, player_id, start, end)
...
if __name__ == '__main__':
    ...
    log_data = get_log_data('https://dps.report/d5cQ-20220517-193416_vg')
